...
## 和原 Claude Code 方案的关系

这个项目借鉴了 “cheap coworker” 模式：

- Claude Code 里用 `CLAUDE.md`
- Codex 里用 `AGENTS.md`
- Claude 的 `ask-kimi` 对应这里的 `ask-worker`
- Claude 的 `kimi-write` 对应这里的 `draft-worker`

差异：

- 本项目面向 Codex
- 默认支持 DeepSeek/Kimi/Ollama 等 OpenAI-compatible provider
- `draft-worker` 默认只生成草稿，不直接覆盖目标文件
- 增加了输入大小上限和 Windows 一键安装脚本

## 视频演示脚本

1. 展示普通 Codex 需要读多个大文件。
2. 安装项目：
...
3. 运行 dry-run：
   ...
   ```powershell
   ask-worker --paths "backend/**/*.js" "frontend/src/**/*.jsx" --question "梳理数据流" --dry-run
   ```

4. 去掉 `--dry-run`，展示 worker 返回摘要和 token 统计。
5. 让 Codex 基于摘要做一个小改动。
6. 展示 `draft-worker` 生成 `.worker-drafts/...`。
7. 总结边界：worker 负责读和起草，Codex 负责判断和验证。

## 路线图

- Codex Skill 打包
- MCP server，把 `ask_worker` 和 `draft_file` 做成一等工具
- Plugin 版本，打包技能、MCP、hooks 和安装流程
- Token/费用监控面板
